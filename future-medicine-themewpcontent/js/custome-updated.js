	...
	/*=====Update js 24-07-2018 ====*/

	jQuery(document).ready(function(){

		jQuery(window).scroll(function(){
	    var sticky = jQuery('.stickyHeader'),
	        scroll = jQuery(window).scrollTop();

	    if (scroll >= 400){
	    	sticky.addClass('fixed-top');
	    	jQuery('.upper-nav').addClass('upper-nav-dnone');
	    	jQuery('.site-logo').addClass('site-logosticky');
	    	jQuery('.lower-nav').addClass('lower-navsticky');
	    	jQuery('.main-menu-wrapper').addClass('main-menu-wrappersticky');
	    }
	    else{
	    	sticky.removeClass('fixed-top');
	    	jQuery('.upper-nav').removeClass('upper-nav-dnone');
	    	jQuery('.site-logo').removeClass('site-logosticky');
	    	jQuery('.lower-nav').removeClass('lower-navsticky');
	    	jQuery('.main-menu-wrapper').removeClass('main-menu-wrappersticky');
	  	} 
  ...
  jQuery(document).ready(function(){
    ...
    jQuery(window).scroll(function(){
      var stickyHmobile = jQuery('.mobile-navigation'),
          scrollHmobile = jQuery(window).scrollTop();

      if (scrollHmobile >= 50){
        stickyHmobile.addClass('addShadowMheader');
      }
      else{
        stickyHmobile.removeClass('addShadowMheader');
...
;(function (factory) {
    ...
    function scrollbarWidth () {
        ...
        var tempEl  = jQuery('<div class="scrollbar-width-tester" style="width:50px;height:50px;overflow-y:scroll;top:-200px;left:-200px;"><div style="height:100px;"></div>'),
            width   = 0,
            widthMinusScrollbars = 0;

        jQuery('body').append(tempEl);

        width = jQuery(tempEl).innerWidth(),
        widthMinusScrollbars = jQuery('div', tempEl).innerWidth();
    ...
    function SimpleBar (element, options) {
        ...
        this.options = jQuery.extend({}, SimpleBar.DEFAULTS, options);
    ...
    SimpleBar.prototype.init = function () {
        ...
        if (this.options.autoHide) {
            this.$el.on('mouseenter', jQuery.proxy(this.flashScrollbar, this));
        }

        this.$scrollbar.on('mousedown', jQuery.proxy(this.startDrag, this));
        this.$scrollContentEl.on('scroll', jQuery.proxy(this.startScroll, this));
    ...
    SimpleBar.prototype.startDrag = function (e) {
        ...
        jQuery(document).on('mousemove', jQuery.proxy(this.drag, this));
        jQuery(document).on('mouseup', jQuery.proxy(this.endDrag, this));
    };
    ...
    SimpleBar.prototype.endDrag = function () {
        jQuery(document).off('mousemove', this.drag);
        jQuery(document).off('mouseup', this.endDrag);
    };
    ...
    jQuery(window).on('load', function () {
        ...
        jQuery('[data-simplebar-direction]').each(function () {
            jQuery(this).simplebar();
        });
    });
     ...
     * Plugin definition
     */
    var old = jQuery.fn.simplebar;

    jQuery.fn.simplebar = function (options) {
        var args = arguments,
            returns;
        ...
        if (typeof options === 'undefined' || typeof options === 'object') {
            return this.each(function () {
                ...
                if (!jQuery.data(this, 'simplebar')) { jQuery.data(this, 'simplebar', new SimpleBar(this, options)); }
            });
        ...
        // treat this as a call to a public method.
            ...
            this.each(function () {
                var instance = jQuery.data(this, 'simplebar');
                ...
                if (options === 'destroy') {
                  jQuery.data(this, 'simplebar', null);
                }
            });
    ...
    jQuery.fn.simplebar.Constructor = SimpleBar;


    /**
     * No conflict
     */
    jQuery.fn.simplebar.noConflict = function () {
        jQuery.fn.simplebar = old;
        return this;
    };

}));

jQuery('#myElement').simplebar();
